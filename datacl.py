...
def get_azure_data(data):
    water,light,soil,absoil,temp,humidity,nitro,phos,pott,flap=data.split(" ")
    ...
    time=str(h)+":"+ct+" "+t
    print(time)
    row = [temp,humidity,light,water,soil,absoil,nitro,phos,pott,flap,time]
    return row
...
def len_of_excel():     
    ...
    if (exists('data.xlsx')==False):
        ...
        sheet.title = 'Sheet1'
        row=1
        new_row = ['temp','humidity','light','soil','absoil','water','nitro','phos','pott','flap','time']
